...
namespace Launchpad.Launcher
{
    internal sealed class ProtocolHandler
    {
        ...
        public string PostToForm(string rawRemoteURL, string username, string password)
        {
            if (bUseHTTP)
            {
                return HTTP.PostToForm(rawRemoteURL, username, password);
            }
            else
                return "";
        }
        ...
        public string GetRemoteManifestChecksum( string WhichManifest )
        {
            if ( bUseHTTP )
            {
                return HTTP.GetRemoteManifestChecksum( WhichManifest );
            }
            else
            {
                return FTP.GetRemoteManifestChecksum( WhichManifest );
            }
        }
